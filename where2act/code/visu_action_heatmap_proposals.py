...
object_urdf_fn = '../urdf/selected_data/faucet/%s/mobility.urdf' % eval_conf.shape_id
...
with torch.no_grad():
    ...
    # result_pccolors = pccolors * (1 - np.expand_dims(pred_action_score_map, axis=-1)) + resultcolors * np.expand_dims(pred_action_score_map, axis=-1)
    result_pccolors = resultcolors
    utils.export_pts_color_obj(fn,  pc[0].cpu().numpy(), result_pccolors)
    ...
    def plot_figure(idx, up, forward, result):
        ...
        rotmat[:3, 1] = left
        rotmat[:3, 2] = up
        rotmat[:3, 3] = position_world - up * robot.len_grasp_to_root
