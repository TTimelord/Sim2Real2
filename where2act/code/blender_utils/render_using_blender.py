...
def render_box(out_fn, box):
    ...
    with open(out_fn + '.obj', 'w') as fout:
        fout.write('mtllib %s\n' % (out_fn.split('/')[-1] + '.mtl'))
        v = (qrot(box[6:].unsqueeze(dim=0).repeat(8, 1), cube_v_torch * box[3:6].unsqueeze(dim=0)) + box[:3].unsqueeze(
            dim=0)).numpy()
        for i in range(8):
            fout.write('v %f %f %f\n' % (v[i, 0], v[i, 1], v[i, 2]))
        for i in range(6):
    ...
    with open(out_fn + '.obj', 'w') as fout:
        ...
        for j in range(boxes.shape[0]):
            v = (qrot(boxes[j, 6:].unsqueeze(dim=0).repeat(8, 1),
                      cube_v_torch * boxes[j, 3:6].unsqueeze(dim=0)) + boxes[j, :3].unsqueeze(dim=0)).numpy()
            for i in range(8):
                fout.write('v %f %f %f\n' % (v[i, 0], v[i, 1], v[i, 2]))
            for i in range(6):
                fout.write('usemtl f%d\n' % i)
...
def render_box_with_rot_mat(out_fn, box):
    ...
    with open(out_fn + '.obj', 'w') as fout:
        fout.write('mtllib %s\n' % (out_fn.split('/')[-1] + '.mtl'))
        v = (torch.matmul(cube_v_torch * box[3:6].unsqueeze(dim=0), box[6:].reshape(3, 3).permute(1, 0)) + box[
                                                                                                           :3].unsqueeze(
            dim=0)).numpy()
        for i in range(8):
            fout.write('v %f %f %f\n' % (v[i, 0], v[i, 1], v[i, 2]))
