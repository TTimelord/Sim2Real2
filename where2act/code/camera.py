...
class Camera(object):
    ...
    def __init__(self, env, near=0.1, far=100.0, image_size=448, dist=2.0, \
        ...
        if random_position:
            ...
            theta = (np.random.random()*2-1) * np.pi/3 + np.pi # -pi/3 ~ pi/3 + pi
    ...
    def get_movable_link_mask(self, link_ids):
        link_seg = self.camera.get_visual_actor_segmentation()[..., 1] # mlq: change to function in sapien2
