...
parser.add_argument('--out_dir', type=str, default='/home/guest2/Documents/Sim2Real2/where2act/data')
...
parser.add_argument('--no_wait_for_still', action='store_true', default=False, help='do_not_wait_for_still [default: False]')
parser.add_argument('--zero_gravity', action='store_true', default=False, help='set gravity to zero [default: False]')
...
# setup env
env = Env(flog=flog, show_gui=(not args.no_gui), zero_gravity=args.zero_gravity)
# env = Env(flog=flog, show_gui=False, zero_gravity=args.zero_gravity)
...
##### mlq: pointcloud visualization
# import open3d as o3d
# pcd = o3d.geometry.PointCloud()
# pcd.points = o3d.utility.Vector3dVector(points.reshape(points.shape[0]*points.shape[1], 3))
...
object_link_ids = env.all_link_ids
gt_movable_link_mask = cam.get_movable_link_mask(object_link_ids)
Image.fromarray((gt_movable_link_mask>0).astype(np.uint8)*255).save(os.path.join(out_dir, 'interaction_mask.png'))
...
robot_material = env.get_material(0.0, 0.0, 0.01)
...
try:
    ...
    rgb_final_pose, _, _ = cam.get_observation()
    Image.fromarray((rgb_final_pose*255).astype(np.uint8)).save(os.path.join(out_dir, 'at_target_pose.png'))
...
if success:
    if env.target_object_part_joint_id == -1:
        out_info['result'] = 'CONTACT_ON_BASE'
    else:
