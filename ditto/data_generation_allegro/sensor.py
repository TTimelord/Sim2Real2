...
from active_light_sensor import AllegroActiveLightSensor

class AllegroSensor(object):

    def __init__(self, env: AllegroEnv, phi=np.pi/5, theta=np.pi, random_position=False, fixed_position=True):
        builder = env.scene.create_actor_builder()
        camera_mount_actor = builder.build_kinematic()
        self.env = env

        self.sensor = AllegroActiveLightSensor(
            name="sensor",
            scene=env.scene,
