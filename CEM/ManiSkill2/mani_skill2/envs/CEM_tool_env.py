...
from mani_skill2.envs.fixed_xmate3_tool_env import (
    FixedXmate3RobotiqToolEnv,
)
...
@register_gym_env("CEM_tool-v0", max_episode_steps=500)
# @register_gym_env_for_tmu("CEM_@-v0")
class CEMToolEnv(FixedXmate3RobotiqToolEnv):
    ...
    def _initialize_agent(self):
        ...
        qpos = np.array([-0.488665,0.340129,-1.14341,1.18789,0.346452,1.73497,0.5321,0.065,0.065]) # drawer
        # qpos = np.array([-0.488665,0.340129,-1.14341,1.18789,0.346452,1.73497,-1,0.0,0.0]) # laptop and 
        
        # qpos = np.array([-0.488665,0.340129,-1.14341,1.18789,1.8,1.73497,0.5321,0.0,0.0]) # drawer with tool
    ...
    def compute_dense_reward(self):
        ...
        for contact in contacts:
            if (
                ...
                if (
                    (contact.actor0 == self.agent.tool_link
                    and contact.actor1 == self.target_link)
                    or
                    (contact.actor0 == self.target_link
                    and contact.actor1 == self.agent.tool_link)
                ):
                    contact_force = []
        ...
        tool_frame_translation = self.agent.tool_link.get_pose().p
        ...
        close_to_target_link_rew = -np.clip(np.linalg.norm(tool_frame_translation - digital_twin_target_center)**2, a_min=0.0, a_max=np.inf) # move to target_link. [-oo, 0]
        ...
        reward_dict = {
            ...
            "robot_q_vel_reward": robot_q_vel_rew * 0.03, # 0.3,
            "robot_q_acc_reward": robot_q_acc_rew * 0.01, # 0.02,
